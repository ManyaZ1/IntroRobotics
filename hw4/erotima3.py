...
##############################################################################
#  4) Closed-form M, c, g using base acceleration + Adjoint
##############################################################################
def skew_6(x):
    """
    x= [w(3), v(3)] => 6x6 ad_x
...
def animate_trajectory_control():
    """
    PD+feedforward on the 4th joint => sinusoid.  Smaller step => more stable.
    """
    dt= 0.0015
    T_sim= 1.5
    steps= int(T_sim/dt)
    ...
    for k in range(steps):
        ...
        # desired
        q_des= np.array([0,0,0,  A*np.sin(omega*t)])
        qdot_des= np.array([0,0,0, A*omega*np.cos(omega*t)])
        qddot_des= np.array([0,0,0, -A*(omega**2)*np.sin(omega*t)])
        ...
        e= q_des- q
        edot= qdot_des- qdot
        feedforward= Mq@ qddot_des
        feedback= Kp* e + Kd* edot
        tau= feedforward + c_ + feedback
...
def animate_trajectory_open_loop():
    dt = 0.01
    T_sim = 5.0
    steps = int(T_sim/dt)
    fig = plt.figure()
    ...
    x = np.zeros(8)
    # List to store the EE positions at each step
    ee_trajectory = []  

    for k in range(steps):
        ax.clear()
        a = np.sin(2*np.pi/T_sim *dt*k)
        qddot = np.array([0,0,0,a])
        tau = inverse_dynamics(x[:4], x[4:8], qddot)
        x = step_state(x, tau, dt)

        # Forward kinematics to get link frames
        link_origins = forward_kinematics_link_origins(x[:4])

        # The end‐effector is link_origins[-1]
        ee_pos = link_origins[-1]
        ee_trajectory.append(ee_pos.copy())  # store for the trail
        ...
        # plot the trail (the entire path so far)
        if len(ee_trajectory) > 1:
            arr = np.array(ee_trajectory)
            ax.plot(arr[:,0], arr[:,1], arr[:,2], 'r-', label="EE Path")
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_xlim(-0.5, 0.5)
...
if __name__=="__main__":
    # 1) Animate sinusoidal PD control on 4th joint
    animate_trajectory_control()
    # 2) Animate open-loop trajectory
    animate_trajectory_open_loop()
