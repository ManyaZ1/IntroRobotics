...
def can_grasp(p_effector, p_target, grasp_threshold=1e-2):
    distance = np.linalg.norm(p_effector - p_target)
    return distance < grasp_threshold

def can_release(p_effector, p_target, release_threshold=1e-2):
    distance = np.linalg.norm(p_effector - p_target)
